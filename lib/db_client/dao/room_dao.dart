...
class RoomDao extends DatabaseAccessor<DbClient> with _$RoomDaoMixin {
  ...
  Future<RoomEntry?> selectRoom(String roomId) {
    return (select(
      roomTable,
    )..where((t) => t.id.equals(roomId))).getSingleOrNull();
  }
  ...
  Future<List<RoomMemberEntry>> getListMember(String chatRoomId) {
    return (select(
      roomMemberTable,
    )..where((t) => t.chatRoomId.equals(chatRoomId))).get();
  }
  ...
  Future<void> deleteMember(String chatRoomId, String userId) {
    return (delete(roomMemberTable)..where(
          (t) => t.chatRoomId.equals(chatRoomId) & t.userId.equals(userId),
        ))
        .go();
  }
}
