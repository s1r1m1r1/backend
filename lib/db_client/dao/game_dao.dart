...
class GameDao extends DatabaseAccessor<DbClient> with _$GameDaoMixin {
  ...
  Future<List<CharacterEntry>> getListCharacter({required String userId}) {
    final query = characterTable.select();
    query.where((t) => t.userId.equals(userId));
    return query.get();
  }

  Future<CharacterEntry?> updateCharacter(
    CharacterTableCompanion characterTableCompanion,
  ) async {
