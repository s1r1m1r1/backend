...
void main() {
  ...
  LetterTableCompanion sampleLetter({
  ...
  test('insertRow inserts and returns the letter', () async {
    final user = await db.userDao.insert(
      UserTableCompanion.insert(email: 'name@name.net', password: '123'),
    );
    final room = await db.roomDao.insertRoom(
      RoomTableCompanion.insert(name: 'Test Room'),
    );
    if (room == null) {
    ...
    final inserted = await dao.insertRow(
      sampleLetter(
        chatRoomId: userInRoom.chatRoomId,
        senderId: userInRoom.userId,
      ),
    );
  ...
  test(
    'getLettersByChannel returns only letters for the given channel',
    () async {
      final user = await db.userDao.insert(
        UserTableCompanion.insert(email: 'name@name.net', password: '123'),
      );
      final room1 = await db.roomDao.insertRoom(
        RoomTableCompanion.insert(name: 'Test Room 1'),
      );
      final room2 = await db.roomDao.insertRoom(
        RoomTableCompanion.insert(name: 'Test Room 2'),
      );
      ...
      await dao.insertRow(
        sampleLetter(
          chatRoomId: userInRoom1.chatRoomId,
          senderId: userInRoom1.userId,
          content: 'A',
        ),
      );
      await dao.insertRow(
        sampleLetter(
          chatRoomId: userInRoom2.chatRoomId,
          senderId: userInRoom2.userId,
          content: 'B',
        ),
      );
      final letters = await dao.getListLetter(
  ...
  test(
    'deleteLettersByChannel removes all letters for the given channel',
    () async {
      final user = await db.userDao.insert(
        UserTableCompanion.insert(email: 'name@name.net', password: '123'),
      );
